...
const SESSION_COOKIE = HOSTED_HTTPS
  ? '__Host-mf_admin_session'
  : 'mf_admin_session';
...
function getAuthConfiguration({ requirePassword = false } = {}) {
  ...
  const bcryptCost = Number.parseInt(passwordHash?.slice(4, 6), 10);
  if (
    requirePassword &&
    (!passwordHash
...
function hasTrustedOrigin(req) {
  ...
  if (!origin) return !isProductionRequest();
  ...
  return expectedOrigins.has(parsedOrigin.origin.toLowerCase());
}
