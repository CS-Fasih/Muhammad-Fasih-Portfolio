...
async function countActivityLoves(activityId) {
  return ActivityLove.countDocuments({ activity: activityId, reactionVersion: 2 }).maxTimeMS(5000);
}
...
async function getActivityLoveCounts(activityIds) {
  ...
  const counts = await ActivityLove.aggregate([
    { $match: { activity: { $in: activityIds }, reactionVersion: 2 } },
...
async function getViewerLovedActivityIds(activityIds, visitorToken) {
  if (!activityIds.length || !visitorToken) return new Set();

  const { hashVisitorForActivity } = require('./visitor');
  const filters = activityIds.map((activityId) => ({
    activity: activityId,
    reactionVersion: 2,
    visitorHash: hashVisitorForActivity(visitorToken, activityId),
  }));
  const reactions = await ActivityLove.find({ $or: filters })
    .select({ activity: 1, _id: 0 })
    .maxTimeMS(5000)
    .lean();

  return new Set(reactions.map(({ activity }) => String(activity)));
}

function serializeActivity(document, { admin = false, loves, viewerLoved = false } = {}) {
  ...
  activity.viewerLoved = Boolean(viewerLoved);

  if (!admin) {
    activity.images = (activity.images || [])
      .filter(({ url, publicId }) => cloudinaryUrlMatchesPublicId(url, publicId))
      .map(({ url, alt }) => ({ url, alt }));
    if (!isWebUrl(activity.externalLink)) {
      delete activity.externalLink;
      delete activity.externalLinkLabel;
    }
  }
